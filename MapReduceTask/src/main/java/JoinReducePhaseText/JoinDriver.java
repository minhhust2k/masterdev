package main.java.JoinReducePhaseText;
...
//            if (!recordFields[0].equals("job")) {
            try {
                ...
                context.write(recordKey, new Text(record.toString()));
            } catch (Exception e) {
...
//            if (!recordFields[0].equals("id")) {
    ...
    public static class JoinReducer extends Reducer<JobKey,
        ...
        public void reduce(JobKey key, Iterable<Text> values,
            ...
            for (Text v : values) {
                if (key.recordType.equals(JobKey.SALARY_RECORD)) {
                    ...
                    output.append(v.toString()).append(key.job.toString()).append(",").append(salary).append("\n");
                }
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "JoinReducePhaseText");
